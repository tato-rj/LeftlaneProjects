      ...
      <div class="col-12 mb-2">
        <div class="d-flex justify-content-between px-3 py-2 bg-light text-muted rounded">
          ...
          <div class="text-right text-brand">
            @can('update', $composer)
            <a href="/piano-lit/composers/{{$composer->id}}">edit</a> | <a href="" data-name="{{$composer->name}}" data-url="/piano-lit/composers/{{$composer->id}}" data-toggle="modal" data-target="#delete-modal" class="delete">delete</a>
            @else
            <a href="/piano-lit/composers/{{$composer->id}}">view details</a>
            @endcan
          </div>
        </div>
      </div>
...
@include('projects/pianolit/components/modals/delete', ['model' => 'composer'])

@component('projects/pianolit/components/modals/add', ['model' => 'composer'])
<form method="POST" action="/piano-lit/composers">
  {{csrf_field()}}
  {{-- Name --}}
  ...
  <div class="form-row form-group">
    <div class="col">
      ...
      @include('projects/pianolit/components/feedback', ['field' => 'nationality'])
    </div>
    <div class="col">
      <select class="form-control {{$errors->has('period') ? 'is-invalid' : ''}}" name="period" >
        ...
        <option value="{{$period}}" {{ old('period') == $period ? 'selected' : ''}}>{{ucfirst($period)}}</option>
        @endforeach
      </select>
