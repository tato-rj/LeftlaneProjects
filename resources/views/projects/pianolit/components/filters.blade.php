<form method="GET" action="{{\URL::current()}}" class="form-inline">
  ...
  <select class="form-control form-control-sm mr-2" name="{{str_singular($filter)}}" onchange="this.form.submit()">
    ...
    @if($filter == 'composers')
      @foreach(\App\Projects\PianoLit\Composer::all() as $composer)
      <option value="{{$composer->id}}" {{(request('composer') == $composer->id) ? 'selected' : ''}}>{{ucwords($composer->name)}}</option>
      @endforeach
    @else
      @foreach(\App\Projects\PianoLit\Tag::$filter()->get() as $field)
  ...
  <div class="btn-group btn-group-toggle mr-2" title="Show only pieces created by me">
    <label class="btn btn-light {{(request('creator_id') == auth()->guard('pianolit-admin')->user()->id) ? 'active' : ''}}">
      <input type="checkbox" name="creator_id" autocomplete="off" value="{{auth()->guard('pianolit-admin')->user()->id}}" onchange="this.form.submit()" {{(request('creator_id') == auth()->guard('pianolit-admin')->user()->id) ? 'checked' : ''}}><i class="fas fa-user"></i>
    </label>
  </div>
