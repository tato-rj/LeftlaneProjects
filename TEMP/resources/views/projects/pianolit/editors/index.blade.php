      ...
      <div class="col-12 mb-2">
        <div class="d-flex justify-content-between px-3 py-2 bg-light text-muted rounded">
          ...
          <div class="text-right text-brand">
            <a href="{{route('piano-lit.editors.edit', $editor->id)}}">edit</a> | <a href="" data-name="{{$editor->name}}" data-url="{{route('piano-lit.editors.destroy', $editor->id)}}" data-toggle="modal" data-target="#delete-modal" class="delete">delete</a>
          </div>
        </div>
      </div>
      @endforeach
...
@include('projects/pianolit/components/modals/delete', ['model' => 'editor'])

@component('projects/pianolit/components/modals/add', ['model' => 'editor'])
<form method="POST" action="/piano-lit/editors">
  {{csrf_field()}}
  {{-- Name --}}
  ...
  <div class="form-group">
    ...
    @include('projects/pianolit/components/feedback', ['field' => 'email'])
  </div>
  {{-- Password --}}
  <div class="form-group{{ $errors->has('password') ? ' has-error' : '' }}">
    <input id="password" type="password" placeholder="Password" class="form-control" name="password" required>
    @include('projects/pianolit/components/feedback', ['field' => 'password'])
  </div>
  <div class="form-group">
