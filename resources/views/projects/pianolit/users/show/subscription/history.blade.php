<table class="table table-striped table-borderless">
  <tbody>
  	@foreach($history as $field => $value)
	  	@if($field != 'in_app')
	  	@include('projects/pianolit/users/show/list-item', ['title' => ucfirst(str_replace('_', ' ', $field)), 'value' => $value])
	  	@endif
  	@endforeach
  </tbody>
</table>
				...
				<table class="table table-hover table-sm table-borderless m-0">
					<tbody>
						@foreach($receipt as $field => $value)
							@include('projects/pianolit/users/show/list-item', ['title' => ucfirst(str_replace('_', ' ', $field)), 'value' => ucfirst($value)])
						@endforeach
					</tbody>
				</table>
			</div>
